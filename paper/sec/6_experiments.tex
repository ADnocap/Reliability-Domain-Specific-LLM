...
\textbf{Qwen3-8B (ours)} & \multicolumn{3}{c}{\textbf{82.2\%} (10-fold CV)} \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{Independent holdout evaluation (54 questions, no overlap with training data). Our fine-tuned 8B model results are pending.}
...
\midrule
Qwen3-8B (base)        & \multicolumn{3}{c}{\TODO{Pending}} \\
\textbf{Qwen3-8B (ours)} & \multicolumn{3}{c}{\TODO{Pending}} \\
\bottomrule
\end{tabular}
\end{table}
...
On the independent holdout (Table~\ref{tab:baselines_holdout}), Claude Sonnet 4.6 achieves 81.1\% and o3-mini 77.1\%. Gemini 3.1 Pro achieves 82.6\% on the 23 questions where answer extraction succeeded, though the high extraction failure rate (31/54 = 57.4\%) limits the reliability of this estimate. Our fine-tuned model's holdout results are pending.

\TODO{Finish/chnage this when eval jobs are done}
...
\TODO{Present before/after comparison on GSM8K, MATH, or other standard reasoning benchmarks. Discuss whether domain-specific SFT caused catastrophic forgetting or whether the chain-of-thought training transferred positively to general math reasoning.}
...
\TODO{Qualitative analysis of what the fine-tuned model gets right vs. wrong. Breakdown by reliability engineering sub-topic (Weibull, Bayesian, system reliability, etc.). Common failure modes: multi-step Bayesian problems, competing risks, edge cases with truncated distributions.}
