...
We digitized 32 chapters from two reliability engineering textbooks---\emph{Applied Reliability} (3rd ed.) by Tobias and Trindade~\cite{tobias2012applied} and \emph{Reliability Engineering} (3rd ed.) by Elsayed~\cite{elsayed2012reliability}---using Mistral OCR, converting them to structured markdown. The textbooks cover exponential and Weibull lifetime distributions, mean time to failure (MTTF) and hazard functions, Bayesian inference, series-parallel and $k$-out-of-$n$ system reliability, reliability demonstration testing, and acceptance sampling.
