...
\node[proc, below=1.2cm of gen, text width=4.7cm] (cleanproc) {Merge, classify, split multi-part,\\deduplicate, standardize answers};
...
% seed feeds into cleaning via left margin (no crossing)
\draw[arrow, rounded corners=5pt]
    (seed.south) -- ++(0,-0.35) -- ++(-6.8,0) |- (cleanproc.west);
...
\draw[arrow] (para) -| (v4);
% master feeds down into both augmentation paths
\draw[arrow] (master.south) -- ++(0,-0.35) -| (hard);
\draw[arrow] (master.south) -- ++(0,-0.35) -| (para);
...
\caption{Data pipeline overview. Blue: dataset milestones; orange: processing; green: LLM-based generation/verification; red: training. Seed questions feed both the generation stage and the cleaning stage (routed via the left margin to avoid crossing Stage~2 nodes). \TODO{FIX THE FUCKING ARROWS}
}
\label{fig:pipeline}
\end{figure*}
...
A key motivation for cross-model verification is our empirical observation that same-model answer consistency, as used in prior work~\cite{yu2024cotselfinst}, is trivially satisfied for self-instruct-generated questions. Table~\ref{tab:ceiling} shows that baseline models achieve 92--97\% accuracy on synthetic questions while scoring only 68--76\% on the 56 textbook-extracted seed problems.

\begin{table}[t]
\centering
\small
\caption{Accuracy of baseline models on textbook-extracted seed questions (56) vs.\ self-instruct-generated questions, revealing the systematic difficulty gap.}
\label{tab:ceiling}
\begin{tabular}{lcc}
\toprule
...
\centering
\small
\caption{Dataset versions and their composition. All entries are numeric QRA triplets with verified answers.}
\label{tab:datasets}
\begin{tabular}{llrl}
\toprule
Version & File & Size & Source \\
\midrule
v1 (cleaned) & \texttt{cleaned.jsonl} & 215 & 98 textbook + 158 verified \\
v2 (+ hard)  & \texttt{v2.jsonl}      & 280 & v1 + 65 hard generated \\
v3 (+ para.) & \texttt{v3.jsonl}      & 501 & v2 + 221 paraphrased \\
\textbf{v4 (+ para.)} & \texttt{\textbf{v4.jsonl}} & \textbf{866} & \textbf{v2 + 586 paraphrased} \\
\bottomrule
\end{tabular}
\end{table}
